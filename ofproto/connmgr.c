...
#include "lib/rconn.h"
#include "lib/vconn-provider.h"
#include <string.h>
//char *con1="m1-s6<->tcp:127.0.0.1:6653";
//char *con2="m1-s6<->tcp:127.0.0.1:6663";
#define ARRAY_SIZE(x) (sizeof(x)/sizeof((x)[0]))
char *con[]={"tcp:127.0.0.1:6653","tcp:127.0.0.1:6663"};
int connum=0;
...
connmgr_set_controllers(struct connmgr *mgr,
                        ...
                        size_t n_controllers, uint32_t allowed_versions)//yxw
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    connum=n_controllers;
    for (i = 0; i < n_controllers; i++) {
        VLOG_INFO("setcon=%d",i);
        ...
        if (!vconn_verify_name(c->target)) {
            ...
            if (!ofconn) {
                ...
                add = true;
                if(i==0){
                    VLOG_INFO("con[0]=%s",c->target);
                }
                /*
                else if(i==1){
                    con[i]=c->target;
                    VLOG_INFO("con[1]=%s",con[i]);
                }
                else if(i==2){
                    con[i]=c->target;
                    VLOG_INFO("con[2]=%s",con[i]);
                }
                else if(i==3){
                    con[i]=c->target;
                    VLOG_INFO("con[3]=%s",con[i]);
                }
                */
                else{
                    con[i-1]=c->target;
                    VLOG_INFO("con[%d]=%s",i,con[i-1]);
                }              
                       ...
                       const struct ofproto_packet_in *pin)
{
    ...
    int choosen_con=0;
    int number=0;
    //char *choosen_name = "m1-s6<->tcp:127.0.0.1:6653";
    char *choosen_name = "";
    srand((int)time(0));
    number = rand() % 101;
    int threshold=0;
    for(int i=0; i<connum-1; i++){
        VLOG_INFO("arr[%d]=：%d  num=%d con=%s\n",i,arr[i],number,con[i]); 
    }
    for(int i=0; i<connum-1; i++){
        threshold+=arr[i];
        if(threshold>=number){
            choosen_name = xasprintf("%s<->%s", mgr->name, con[i]);
            break;
        }
    }
    //VLOG_INFO("arr=：%d  num=%d\n",arr[0],number); 
    /*
    if (arr[0] > number){
        choosen_con=1;
        //choosen_name = "m1-s6<->tcp:127.0.0.1:6653";
        choosen_name = xasprintf("%s<->%s", "m1-s6", con[1]);
    }
    else if(arr[0]+arr[1] > number){
        choosen_con=2;
        //choosen_name = "m1-s6<->tcp:127.0.0.1:6663";
        choosen_name = xasprintf("%s<->%s", "m1-s6", con[2]);
    }
    else{
        choosen_con=3;
        //choosen_name = "m1-s6<->tcp:127.0.0.1:6663";
        choosen_name = xasprintf("%s<->%s", "m1-s6", con[3]);
    }
    */
    LIST_FOR_EACH (ofconn, node, &mgr->all_conns) {
        enum ofp_packet_in_reason reason = wire_reason(ofconn, pin);
        yxwid++;
        VLOG_INFO("controller_run=%d processed!name=%s",yxwid,rconn_get_name(ofconn->rconn));
        if (ofconn_wants_packet_in_on_miss(ofconn, pin)
            && ofconn_receives_async_msg(ofconn, OAM_PACKET_IN, reason)
            //&& ofconn->controller_id == pin->controller_id && yxwid==choosen_con) {
            && ofconn->controller_id == pin->controller_id && strcmp(choosen_name,rconn_get_name(ofconn->rconn))==0) {
            schedule_packet_in(ofconn, *pin, reason);
            //VLOG_INFO("yxwid=%d processed!",yxwid);
            VLOG_INFO("yxwid=%d,controller=%s processed!",yxwid,choosen_name);
        }
    }
}
